...
import { TaskTimeLoggedEvent } from '../activity/events';

@Injectable()
export class TimeLogsService {
  constructor(
    @InjectRepository(TimeLog)
    private readonly timeLogsRepo: Repository<TimeLog>,
  ...
  async create(taskId: number, dto: CreateTimeLogDto, userId: number): Promise<TimeLog> {
    ...
    const entry = this.timeLogsRepo.create({
      taskId,
      userId,
      hours: dto.hours,
      date: dto.date,
    });
    const saved = await this.timeLogsRepo.save(entry);
    ...
    return this.timeLogsRepo.findOneOrFail({
      where: { id: saved.id },
      relations: { user: true },
    });
  }
  ...
  async findAllByTask(taskId: number): Promise<{ entries: TimeLog[]; totalHours: number }> {
    const entries = await this.timeLogsRepo.find({
      where: { taskId },
      relations: { user: true },
  ...
  async remove(id: number, userId: number): Promise<void> {
    const entry = await this.timeLogsRepo.findOne({ where: { id } });
    if (!entry) throw new NotFoundException('Time log not found');

    if (entry.userId === userId) {
      await this.timeLogsRepo.remove(entry);
      return;
    }
